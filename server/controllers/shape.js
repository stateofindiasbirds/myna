  ...
  async iucnRedListSpeicies(req, res) {
    ...
    Kinnaur.findAll({
      attributes: [
        "iucnCategory",
        "eBirdScientificName",
        [Sequelize.fn("count", Sequelize.col("eBirdScientificName")), "count"],
      ],
      where: {
        iucnCategory: {
          [Sequelize.Op.in]: [
  ...
  async endemincSpecies(req, res) {
    ...
    Kinnaur.findAll({
      ...
      .then((results) => {
        ...
        // Get the region for every bird and their percentage and their region
        const data = Object.entries(percentages).map(([region, birds]) => {
          return Object.entries(birds).map(([name, percentage]) => ({
            name,
            percentage,
          ...
          const top10Birds = top10BirdArrays.map((ele) => {
            ...
            months.forEach((month) => {
              ...
              const countBySpecies = filteredData.reduce((acc, item) => {
                ...
                const percentage =
                  ((acc[key] ? acc[key].count : 0) / speciesFrequency) * 100;
                acc[key] = acc[key]
                  ? {
