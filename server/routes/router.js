  ...
  const oneYearInSeconds = 60;

  router.get('/geojson/states',  
    cacheMiddleware(geoJSONCacheKeyGenerator, oneYearInSeconds), 
    async (req, res) => {
  ...
  router.get("/location_listing",
    // cacheMiddleware(cacheKeyGenerator), 
    UserController.locationName);
  ...
  router.get("/complete_List_Of_Species", 
    // cacheMiddleware(cacheKeyGenerator), 
  ...
  router.get("/effortsDetails", 
    // cacheMiddleware(cacheKeyGenerator), 
    UserController.effortsDetails);
  router.get("/soibConcernStatus", 
    // cacheMiddleware(cacheKeyGenerator), 
    UserController.soibConcernStatus);

  router.get("/count_iucn_species", 
    // cacheMiddleware(cacheKeyGenerator), 
    UserController.count1);
  router.get("/count_appendix_species", 
    // cacheMiddleware(cacheKeyGenerator), 
    UserController.count2);
  router.get("/count_number_species", 
