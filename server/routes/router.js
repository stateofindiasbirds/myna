...
const cacheKeyGenerator = (req) => {
  ...
  console.log(state,county,locality,start,end);
  const pathname = req.originalUrl.split('?')[0]; 
  if(state && county && locality){
    console.log(`${encodeURIComponent(state)}:${encodeURIComponent(county)}:${encodeURIComponent(locality)}:${encodeURIComponent(pathname)}:${start}:${end}`);
    ...
    return `${encodeURIComponent(state)}:${encodeURIComponent(county)}:${encodeURIComponent(pathname)}:${start}:${end}`;
  }else{
    return
  }
};
...
router.get("/complete_List_Of_Species_gi", cacheMiddleware(cacheKeyGenerator), UserController.completeListOfSpeciesGi);
