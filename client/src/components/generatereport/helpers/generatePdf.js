...
//     const imgWidthHmap = 260; // Fixed width (you can modify this)
//     const xPosHmap = (pageWidthHmap - imgWidthHmap) / 2; // Center horizontally
//     const mapYPosition = 100; // Starting position of the heatmap image
...
//         let imgWidth = pageWidth - 20; // Leave some margin (10 units on each side)
//         let imgHeight = imgWidth / imgAspectRatio;
      
//         // Ensure the image height also fits within the page height
//         if (imgHeight > pageHeight - 80) { // Leave some vertical margin
//           imgHeight = pageHeight - 80;
//           imgWidth = imgHeight * imgAspectRatio;
//         }
      
//         const xPos = (pageWidth - imgWidth) / 2; // Center horizontally
//         const yPos = 20; // Fixed Y position or adjust dynamically as needed
...
const canvas7 = await captureCanvasOld(heatmapRef);
...
const lockPdf = async (jsPDFDoc, pdfName) => {
  ...
  { canvas2 && getSeasonalChartData?.length && pdf.addPage(); }
    ...
    const fifthImage = canvas5?.toDataURL("image/png");
    // console.log("fifthImage",fifthImage);
    // console.log("heatmapRef.current.style.width",heatmapRef.current.style.width)
    if(seventhImg && seventhImg.startsWith("data:image/png")){ 
      // console.log("seventhImg",seventhImg)
      const heatmapImageProperties = pdf.getImageProperties(seventhImg);
      const pageWidthHmap = pdf.internal.pageSize.getWidth();
